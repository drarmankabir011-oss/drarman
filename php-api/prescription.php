...
// ============================================
    ...
    // Authorization: clinical staff or patient owner
    ...
    if (!in_array($payload['role'] ?? '', $allowed)) {
        if (($payload['user_id'] ?? null) != $prescription['patient_id']) {
...
// ============================================
    ...
    $allowed = ['admin','doctor','consultant_doctor','medical_officer','nurse','registrar','assistant_registrar','staff'];
    if (!in_array($payload['role'] ?? '', $allowed)) {
        if (($payload['user_id'] ?? null) != $patient_id) {
            sendError('Forbidden', 403);
        }
    }
...
// ============================================
    ...
    $vq = $conn->prepare("SELECT patient_id FROM visits WHERE id = ? LIMIT 1");
    $vq->bind_param('i', $visit_id);
    $vq->execute();
    $vres = $vq->get_result();
    $visitRow = $vres && $vres->num_rows ? $vres->fetch_assoc() : null;
    $patient_id = $visitRow ? $visitRow['patient_id'] : null;

    $allowed = ['admin','doctor','consultant_doctor','medical_officer','nurse','registrar','assistant_registrar','staff'];
    if (!in_array($payload['role'] ?? '', $allowed)) {
        if ($patient_id == null || ($payload['user_id'] ?? null) != $patient_id) {
            sendError('Forbidden', 403);
        }
    }
...
// ============================================
    ...
    // only clinical prescribers and admin may create prescriptions
    $payload = checkPermission(['admin','doctor','consultant_doctor','medical_officer']);
...
// ============================================
    ...
    // Fetch existing to check author/owner
    $q = "SELECT * FROM prescriptions WHERE id = ?";
    $s = $conn->prepare($q);
    $s->bind_param('i', $prescription_id);
    $s->execute();
    $r = $s->get_result();
    if ($r->num_rows === 0) sendError('Prescription not found', 404);
    $existing = $r->fetch_assoc();

    // Authorization: admin or prescriber (doctor/consultant) or the original author allowed to update
    $allowedRoles = ['admin','doctor','consultant_doctor','medical_officer'];
    if (!in_array($payload['role'] ?? '', $allowedRoles) && ($payload['user_id'] ?? null) != $existing['author_id']) {
        sendError('Forbidden', 403);
    }
...
// ============================================
    ...
    // only admin or senior prescribers may delete prescriptions
